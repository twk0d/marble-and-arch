...
@RestController
@RequestMapping("/api/${api.version}/properties")
...
public class PropertyController {
    ...
    public CompletableFuture<ResponseEntity<Page<PropertySummaryDTO>>> pageableSearch(
            @ModelAttribute PageableFilters filters,
    ...
    @PostMapping("/{propertyUUID}/approve")
    @PreAuthorize("hasAnyRole('ADMIN', 'MANAGER')")
    public CompletableFuture<ResponseEntity<Void>> approveProperty(@PathVariable UUID propertyUUID) {
        CompletableFuture<Void> resultFuture = new CompletableFuture<>();
        var command = new ApprovePropertyCommand(propertyUUID, resultFuture);
        return moduleClient.executeCommandAsync(command)
                .thenApply(result -> ResponseEntity.ok().build());
    }

    @PostMapping("/{propertyUUID}/reject")
    @PreAuthorize("hasAnyRole('ADMIN', 'MANAGER')")
    public CompletableFuture<ResponseEntity<Void>> rejectProperty(@PathVariable UUID propertyUUID) {
        CompletableFuture<Void> resultFuture = new CompletableFuture<>();
        var command = new RejectPropertyCommand(propertyUUID, resultFuture);
        return moduleClient.executeCommandAsync(command)
                .thenApply(result -> ResponseEntity.ok().build());
    }
