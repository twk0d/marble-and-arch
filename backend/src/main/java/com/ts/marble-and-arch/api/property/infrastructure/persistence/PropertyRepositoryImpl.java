...
public class PropertyRepositoryImpl implements IPropertyRepository {
    ...
    private Property toDomain(PropertyJpaEntity jpaEntity) {
        List<Image> images = jpaEntity.getImages().stream()
                .map(img -> new Image(img.getId(), jpaEntity.getId(), img.getUrl(), img.getDescription(), img.isActive()))
                .collect(Collectors.toList());
    ...
    private PropertyJpaEntity toJpaEntity(Property domainProperty) {
        ...
        jpaEntity.setStreet(domainProperty.getAddress().street());
        jpaEntity.setNumber(domainProperty.getAddress().number());
        jpaEntity.setNeighborhood(domainProperty.getAddress().neighborhood());
        jpaEntity.setCity(domainProperty.getAddress().city());
        jpaEntity.setState(domainProperty.getAddress().state());
        jpaEntity.setPostalCode(domainProperty.getAddress().postalCode());
        jpaEntity.setComplement(domainProperty.getAddress().complement());
        ...
        List<ImageJpaEntity> imageJpaEntities = domainProperty.getImages().stream()
                .map(img -> new ImageJpaEntity(img.getId(), jpaEntity, img.getUrl(), img.getDescription(), img.isActive()))
                .collect(Collectors.toList());
