...
const { data: property, pending, error } = useFetch<PropertyDTO>(`/api/v1/property-management/${propertyId}`, {
...
useSeoMeta({
  title: () => property.value ? `${t(`propertyType.${property.value.type}`)} em ${property.value.address.city} - Marble & Arch` : 'Carregando Imóvel...',
  ogTitle: () => property.value ? `${t(`propertyType.${property.value.type}`)} em ${property.value.address.city}` : 'Marble & Arch Imóveis',
  description: () => property.value?.details.description || 'Confira os detalhes deste imóvel exclusivo na Marble & Arch.',
  ogDescription: () => property.value?.details.description || 'Confira os detalhes deste imóvel exclusivo na Marble & Arch.',
                ...
                <template v-else-if="property">
                  ...
                  <div class="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div class="text-body-xs text-neutral-500">Agência Marble & Arch</div>
                    </div>
                  </div>
                </template>
              </UCard>
